...
class IndexerTest extends Tx_Extbase_BaseTestCase {
	...
	public function setUp() {
		...
		$this->indexer->additionalFields = array('title', 'orig_uid', 'orig_pid', 'enddate');
	 ...
	 * @test
	 */
	public function checkAddQueryPartsForAdditionalFields() {
		$now = time();
		$fieldValues = array(
			'tstamp' => $now,
			'crdate' => $now,
		...
		$shouldArray = array(
			'set' => ', @title = \'tolle Überschrift\', @orig_uid = \'213\', @orig_pid = \'423\', @enddate = \'' . $now . '\'',
			'execute' => ', @title, @orig_uid, @orig_pid, @enddate'
		);
